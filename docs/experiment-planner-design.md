...
For L3 in vials/bottles, use the user's editable **25°C-equivalent D5** preset and recorded temperature history. With continuous 25°C, a target collection on September 28 maps to a proposed new culture setup date of September 23. Cooling delays the estimate; actual return to 25°C advances it relative to continued cooling while preserving accumulated delay. Keep the configured collection window. This remains a researcher-selected estimate, not observed L3 staging or a requirement to create a new culture if suitable material exists. If dissection follows collection after a delay, solve for the collection step from that dependency before deriving D0.
...
2. **Deterministic backward planning**: L1 and L3 examples, interval validation, availability, missing-input states, actual/locked anchors, and plan comparison. Accept when D0/egg-age distinctions, temperature-adjusted equivalent D5, delayed laying, absent counts, contradictory constraints, and workday conflicts are covered by tests.
