...
2. Enter the **Base URL**, including `/v1` if your server uses it. Enter the base URL, not the full `/chat/completions` route. Cloud URLs require HTTPS. Local URLs must use localhost or a loopback address; start the local model server first.
...
4. Click **Fetch models** to check the current URL/key and load available model IDs before saving. Search the list and select a model, or enter an ID manually if the provider does not support listing. Opening an already configured connection loads its model list automatically. Fetching uses only connection details and does not send laboratory records or change the saved configuration.
5. Choose **Save and test**. This saves the visible profile and sends a short connection-test request without laboratory records. A successful result confirms that the configured model returned text. **Save connection** also accepts a blank model so the connection can be completed later. A fetched model list confirms API access; **Save and test** separately checks that the selected model can generate a response. Cloud and local profiles have separate saved values; switching the active profile requires saving it.
6. Return to **Assistant**. Choose whether to **Include workspace data**, then use all containers or **Selected containers and sources**. Set an optional target date and choose **Plan L1 dissection** or **Plan L3 dissection** to fill an editable message, or write your own. The target date is included in the sent message when set.
