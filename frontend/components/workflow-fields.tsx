...
export function WorkflowFields({purpose, value, goal, setGoal}: {purpose: Culture['purpose']; value?: Workflow | null; goal: 'score' | 'virgins'; setGoal: (goal: 'score' | 'virgins') => void}) {
  return <section className="form-section"><h3>{t('workflow.title')}</h3>
    <p className="notice">{t(`workflow.${purpose}`)}</p>
    {purpose === 'cross' && <>
      <Field label={t('workflow.goal')}><select value={goal} onChange={e => setGoal(e.target.value as 'score' | 'virgins')}><option value="score">{t('workflow.score')}</option><option value="virgins">{t('workflow.virgins')}</option></select></Field>
...
export function workflowFrom(form: FormData, goal: 'score' | 'virgins', base?: Workflow | null): Workflow {
