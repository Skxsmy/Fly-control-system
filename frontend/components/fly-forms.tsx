...
export function EggBatchForm({culture, now, close, saved}: {culture: Culture; now: string; close: () => void; saved: () => Promise<void>}) {
  return <Modal title={t('eggs.new')} description={t('eggs.newHint')} close={close}><Form close={close} submit={async form => {
    ...
    <Field label={t('container.genotype')} hint={t('eggs.genotypeHint')}><input name="genotype" required defaultValue={culture.genotype}/></Field>
...
export function ContainerForm({state, source, mode, eggBatch, close, saved}: {state: AppState; source?: Culture; mode?: 'transfer' | 'generation' | 'edit'; eggBatch?: EggBatch; close: () => void; saved: (id?: string) => Promise<void>}) {
  ...
  return <Modal title={t(edit ? 'container.edit' : mode === 'transfer' ? 'container.transfer' : mode === 'generation' ? 'container.nextGeneration' : 'common.newContainer')} description={t(mode === 'transfer' ? 'container.transferHint' : mode === 'generation' ? 'container.generationHint' : kind === 'petri_dish' ? 'eggs.dishHint' : kind === 'egg_laying' ? 'eggs.containerHint' : 'container.newHint')} close={close} wide>
    <Form close={close} label={t(edit ? 'common.save' : 'common.create')} submit={async form => {
      ...
      {purpose === 'cross' ? <div className="form-grid"><Field label={t('container.female')}><input name="female_genotype" required defaultValue={source?.female_genotype} readOnly={mode === 'transfer'} list="known-genotypes"/></Field><Field label={t('container.male')}><input name="male_genotype" required defaultValue={source?.male_genotype} readOnly={mode === 'transfer'} list="known-genotypes"/></Field></div> : <Field label={t(kind === 'egg_laying' ? 'eggs.knownGenotype' : 'container.genotype')} hint={kind === 'egg_laying' ? t('eggs.knownGenotypeHint') : undefined}><input key={`${kind}-${batchId}`} name="genotype" required defaultValue={batch?.genotype || source?.genotype} readOnly={mode === 'transfer'} list="known-genotypes"/></Field>}
      ...
      {!mode && <section className="form-section"><h3>{t('container.initialState')}</h3><p className="subtle">{t(kind === 'egg_laying' ? 'eggs.initialHint' : 'container.initialHint')}</p><div className="form-grid">
