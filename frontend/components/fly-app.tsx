...
import { InjectionStartForm, InjectionActionForm, InjectionDetails, isInjectionChild } from './injection-preparation';
...
type DialogState = {kind: 'injection-start'; culture: Culture} | {kind: 'injection-action'; culture: Culture; event: Task} | {kind: 'delete-activity'; culture: Culture; activity: ActivityRecord} | {kind: 'container'; source?: Culture; eggBatch?: EggBatch; mode?: 'transfer' | 'generation' | 'renew' | 'edit'} | {kind: 'action'; culture: Culture; action: string; event?: Task} | {kind: 'reminder'; event?: Task; cultureId?: string; date?: string; batch?: EggBatch} | {kind: 'availability'; date: string} | {kind: 'planner'; culture: Culture} | {kind: 'egg-batch'; culture: Culture} | {kind: 'egg-from'; culture: Culture} | {kind: 'delete'; culture: Culture} | {kind: 'egg-action'; batch: EggBatch; action: 'collect' | 'use' | 'cancel'};
...
const cultureDay = (c: Culture) => isInjectionChild(c) && !c.injection?.started_at ? t('injection.awaitingCollection') : c.status === 'planned' && !isInjectionChild(c) ? t('eggs.notStarted') : t('container.day', {day: c.calendar_day ?? 0});
...
export default function FlyApp() {
  ...
  function perform(e: Task) {
    ...
    if (c && e.kind.startsWith('injection_')) return setDialog({kind: 'injection-action', culture: c, event: e});
    ...
    if (c && e.kind === 'egg_setup') return setDialog({kind: 'action', culture: c, action: 'activate', event: e});
    if (c && e.kind === 'transfer') return setDialog({kind: 'container', source: c, mode: 'transfer'});
    if (c && e.kind === 'stock') return setDialog({kind: 'container', source: c, mode: 'renew'});
    if (c && ['collect', 'tissue', 'cold', 'warm', 'first_instar', 'remove', 'score', 'watch', 'third_instar'].includes(e.kind)) return setDialog({kind: 'action', culture: c, action: e.kind === 'watch' ? 'eclosion' : e.kind, event: e});
  ...
  const todayWork = state ? pending.filter(e => isTodayWork(e, state.now)) : [];
  const overdue = state ? pending.filter(e => isOverdue(e, state.now)) : [];
  ...
  return <div className="fly-app"><a className="skip-link" href="#main">{t('nav.workspace')}</a>
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumb"><strong>{t(`nav.${page}`)}</strong></div><div className="topbar-right"><span className="zone"><Clock3 size={15}/>{state?.settings.timezone}</span></div></header>
      <main id="main">
        ...
        {!state ? <div className="loading"><span className="spinner"/>{t('common.loading')}</div> : <>
          ...
          {page === 'today' && <>
            <div className="metrics">{[{label: 'today.active', value: state.containers.filter(c => c.status === 'active').length, icon: FlaskConical, tone: 'green'}, {label: 'today.due', value: todayWork.length, icon: ListFilter, tone: 'neutral'}, {label: 'today.critical', value: todayWork.filter(e => e.critical).length, icon: Activity, tone: 'amber'}, {label: 'today.cold', value: state.containers.filter(c => c.status === 'active' && c.temperature === 18).length, icon: Snowflake, tone: 'blue'}].map(({label, value, icon: Icon, tone}) => <div className="metric" key={label}><span className={`metric-icon ${tone}`}><Icon size={20}/></span><div><span>{t(label)}</span><strong>{fmtNumber(value)}</strong></div></div>)}</div>
            {state.containers.length === 0 ? <section className="welcome panel"><div className="culture-symbol"><FlaskConical size={38}/></div><h2>{t('today.welcome')}</h2><Button onClick={() => setDialog({kind: 'container'})}><Plus size={18}/>{t('common.newContainer')}</Button></section> : <div className="today-layout"><div>
              {overdue.length > 0 && <section className="panel overdue-panel"><div className="panel-title"><h2><TriangleAlert size={18}/>{t('today.overdue')}</h2><span className="count-badge">{overdue.length}</span></div><TaskList {...taskProps} events={overdue} showDate/></section>}
              <section className="panel"><div className="panel-title"><h2>{t('today.queue')}<span className="count-badge">{todayWork.length}</span></h2><Button variant="ghost" onClick={() => setDialog({kind: 'reminder'})}><Plus size={16}/>{t('common.newTask')}</Button></div>
                <TaskList {...taskProps} events={todayWork} empty={<Empty title={t('today.empty')}/>}/>
              </section>
              ...
              <section className="panel conflict-panel"><div className="panel-title"><h2><TriangleAlert size={18}/>{t('today.conflicts')}</h2></div>
    ...
    {dialog && state && <>
      {dialog.kind === 'injection-start' && <InjectionStartForm culture={dialog.culture} state={state} close={() => setDialog(null)} saved={saved}/>}
      {dialog.kind === 'injection-action' && <InjectionActionForm culture={dialog.culture} state={state} event={dialog.event} close={() => setDialog(null)} saved={saved}/>}
      ...
      {dialog.kind === 'action' && <ActionForm culture={dialog.culture} action={dialog.action} event={dialog.event} now={state.now} linkedPlanned={state.containers.filter(c => c.source_id === dialog.culture.id && isInjectionChild(c) && c.status === 'planned').map(c => c.label)} close={() => setDialog(null)} saved={saved}/>}
...
function Empty({title}: {title: string}) {return <div className="empty"><span className="empty-icon"><Check size={26}/></span><h3>{title}</h3></div>;}
...
function TaskList({events, containers, perform, move, update, select, busy, empty, showDate}: TaskProps & {events: Task[]; empty?: React.ReactNode; showDate?: boolean}) {
  ...
  return <div className="task-list">{events.map(e => {const c = containers.find(c => c.id === e.container_id); const spansDays = e.due.slice(0, 10) !== e.end.slice(0, 10); const target = e.scheduled_at || (e.open_ended && e.pinned && e.due.slice(11, 16) !== '00:00' ? e.due : undefined); return <div className={`task-row ${e.critical && e.status === 'pending' ? 'critical' : ''} ${e.status !== 'pending' ? 'resolved' : ''}`} key={e.id}>
    <div className="task-time">{e.open_ended ? <span>{t('task.ongoingFrom', {date: fmtDate(e.due)})}</span> : (showDate || spansDays) && <span>{fmtDate(e.due)}</span>}<strong>{e.all_day ? t('task.allDay') : <>{fmtTime(e.due)}{!spansDays && e.end !== e.due ? `–${fmtTime(e.end)}` : ''}</>}</strong>{target && <small>{t('task.targetTime', {time: `${target.slice(0, 10) !== e.due.slice(0, 10) ? fmtDate(target) + ' · ' : ''}${fmtTime(target)}`})}</small>}{spansDays && !e.open_ended && <small>→ {fmtDate(e.end)} · {fmtTime(e.end)}</small>}</div>
    <div className="task-body"><div className="task-line"><strong>{taskTitle(e)}</strong>{e.critical && e.status === 'pending' && <span className="pill amber">{t('task.critical')}</span>}{e.timing_review && e.status === 'pending' && <span className="pill amber">{t('eggs.reviewBadge')}</span>}{e.pinned && <span className="pin-dot" title={t('task.pinned')}>•</span>}</div><div className="task-meta">{c ? <button onClick={() => select(c.id)} className="container-link">{c.label}</button> : <span>{t('task.general')}</span>}{c && <span className="genotype-preview">{genotype(c)}</span>}{e.status !== 'pending' && <span>{t(`task.${e.status}`)}</span>}{e.conflict && e.status === 'pending' && <TriangleAlert size={14} className="warning-icon" aria-label={t('task.conflict')}/>}</div></div>
    <div className="task-controls">{e.status === 'pending' && <Button variant="outline" size="sm" disabled={busy || (c?.status === 'planned' && !['egg_setup','offspring_ready'].includes(e.kind))} title={t(e.kind === 'stock' ? 'task.stock' : 'task.markDone')} onClick={() => perform(e)}>{e.kind === 'stock' ? <Plus size={15}/> : <Check size={15}/>}<span>{t(e.kind === 'stock' ? 'task.renew' : 'common.done')}</span></Button>}
      {e.status !== 'cancelled' && (!c || ['active','planned'].includes(c.status)) && <details className="task-menu"><summary aria-label={t('container.operations')}><MoreHorizontal size={20}/></summary><div className="menu-items"><button disabled={busy} onClick={() => move(e)}>{t('task.move')}</button>{e.status === 'pending' && <><button disabled={busy} onClick={() => update(e, {status: 'skipped'})}>{t('task.skip')}</button><button disabled={busy} onClick={() => update(e, {status: 'disabled'})}>{t('task.disable')}</button></>}<button disabled={busy} onClick={() => update(e, {restore: true})}>{t('task.restore')}</button></div></details>}
    </div>
  </div>;})}</div>;
}
...
function Containers({state, search, setSearch, select, create}: {state: AppState; search: string; setSearch: (v: string) => void; select: (v: string) => void; create: () => void}) {
  ...
  return <section className="panel containers-panel"><div className="table-toolbar"><label className="search"><Search size={18}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('common.search')} aria-label={t('common.search')}/></label><select aria-label={t('container.status')} value={status} onChange={e => setStatus(e.target.value)}><option value="active">{t('container.active')}</option><option value="archive">{t('container.archive')}</option><option value="all">{t('container.all')}</option></select><select aria-label={t('container.purpose')} value={purpose} onChange={e => setPurpose(e.target.value)}><option value="all">{t('common.all')}</option>{['stock', 'cross', 'virgin', 'larvae', 'egg_laying', 'dissection', 'imaging', 'other', 'injection'].map(x => <option key={x} value={x}>{t(`purpose.${x}`)}</option>)}</select><label className="check-label"><input type="checkbox" checked={grouped} onChange={e => setGrouped(e.target.checked)}/>{t('container.group')}</label></div>
    <div className="table-scroll"><table><thead><tr>{['id', 'genotype', 'age', 'temperature', 'transfers', 'next'].map(x => <th key={x}>{t(`container.${x}`)}</th>)}</tr></thead><tbody>{rows.map((c, index) => {
      ...
      return <ContainerRows key={c.id} culture={c} now={state.now} next={next} select={select} group={grouped && (index === 0 || genotype(rows[index - 1]) !== genotype(c)) ? genotype(c) : undefined}/>;
...
function ContainerRows({culture: c, now, next, select, group}: {culture: Culture; now: string; next?: Task; select: (id: string) => void; group?: string}) {return <>{group && <tr className="group-row"><th colSpan={6}>{group}</th></tr>}<tr>
  ...
  <td className="genotype-cell"><span>{genotype(c)}</span><small>{t(`purpose.${c.purpose}`)}</small></td><td><strong className="mono">{cultureDay(c)}</strong>{c.kind === 'egg_laying' && c.status !== 'planned' && <small>{eggElapsed(c, now)}</small>}{c.kind === 'petri_dish' && c.status !== 'planned' && <small>{t('eggs.ageShort', {min:fmtNumber(c.egg_age_hours?.[0] || 0), max:fmtNumber(c.egg_age_hours?.[1] || 0)})}</small>}{(!isInjectionChild(c) || c.injection?.started_at) && <small>{fmtDate(c.setup_date)}{(c.kind === 'egg_laying' || isInjectionChild(c)) && c.setup_time ? ` · ${c.setup_time}` : ''}</small>}</td>
  <td><span className={`pill ${c.temperature === 18 ? 'blue' : 'neutral'}`}>{c.temperature === 18 ? <Snowflake size={13}/> : <Thermometer size={13}/>} {c.temperature}°C</span></td><td>{c.kind === 'petri_dish' || isInjectionChild(c) ? '—' : <><span className="mono">{c.transfer_index}{c.kind !== 'egg_laying' && ` / ${c.template.max_transfers}`}</span><small>{t(c.kind === 'egg_laying' && c.status === 'planned' ? 'eggs.adultsPending' : `parents.${c.parents}`)}</small></>}</td><td>{next ? <><span className="next-task">{taskTitle(next)}</span><small className={next.conflict ? 'warning-text' : ''}>{fmtDate(next.due)} · {next.all_day ? t('task.allDay') : fmtTime(next.due)}</small></> : <span className="subtle">—</span>}</td>
</tr></>;}
...
function CultureDetails({culture: c, state, taskProps, dialog, select, askAssistant}: {culture: Culture; state: AppState; taskProps: TaskProps; dialog: (d: DialogState) => void; select: (id: string) => void; askAssistant: (id: string) => void}) {
  ...
  const injectionChild = isInjectionChild(c);
  const batch = state.egg_batches.find(b => b.id === c.egg_batch_id);
  const source = state.containers.find(x => x.id === c.source_id);
  const collectsVirgins = !c.injection && (c.purpose === 'virgin' || (c.purpose === 'cross' && (!c.workflow || c.workflow.cross_goal === 'virgins')));
  return <div className="culture-detail"><div className="detail-badges"><span className="pill green">{t(`status.${c.status}`)}</span><span className="pill neutral">{t(`purpose.${c.purpose}`)}</span><span className={`pill ${c.temperature === 18 ? 'blue' : 'neutral'}`}>{c.temperature}°C</span><Button variant="ghost" onClick={() => dialog({kind: 'container', source: c, mode: 'edit'})}><Settings2 size={16}/>{t('common.edit')}</Button></div>
    <div className="detail-clock"><div><span>{t(injectionChild ? 'injection.elapsed' : 'container.age')}</span><strong>{cultureDay(c)}</strong>{!hourly && !injectionChild && <small>{t('container.effective', {age: fmtNumber(c.effective_age)})}</small>}</div>{hourly && <div><span>{t(c.kind === 'petri_dish' ? 'eggs.ageLabel' : 'eggs.elapsedLabel')}</span><strong>{c.kind === 'petri_dish' ? t('eggs.ageShort', {min:fmtNumber(c.egg_age_hours?.[0] || 0), max:fmtNumber(c.egg_age_hours?.[1] || 0)}) : eggElapsed(c, state.now)}</strong></div>}{c.kind !== 'petri_dish' && !injectionChild && <div><span>{t('container.transfers')}</span><strong>{c.transfer_index}{!hourly && <small> / {c.template.max_transfers}</small>}</strong><small>{t(c.kind === 'egg_laying' && c.status === 'planned' ? 'eggs.adultsPending' : `parents.${c.parents}`)}</small></div>}</div>
    <dl className="detail-facts"><div><dt>{t(injectionChild ? 'injection.inheritedStart' : c.kind === 'petri_dish' ? 'eggs.dishSetup' : c.kind === 'egg_laying' ? c.status === 'planned' ? 'eggs.plannedStart' : 'eggs.actualStart' : 'container.setup')}</dt><dd>{injectionChild && !c.injection?.started_at ? t('injection.awaitingCollection') : <>{fmtDate(c.setup_date, {year: 'numeric', month: 'short', day: 'numeric'})}{c.setup_time ? ` · ${c.setup_time}` : ''}</>}</dd></div><div><dt>{t('container.stage')}</dt><dd>{t(`stage.${c.stage}`)}</dd></div><div><dt>{t('temperature.policy')}</dt><dd>{t(`temperature.${c.temperature_policy}`)}</dd></div>{source && <div><dt>{t('container.source')}</dt><dd><button className="text-link" onClick={() => select(source.id)}>{source.label}</button></dd></div>}</dl>
    {c.source_relation?.startsWith('egg_laying') && <p className="subtle">{t('eggs.sourceRelation')}: {t(c.adult_source === 'parents' || ['egg_laying_transfer','egg_laying_parents'].includes(c.source_relation) ? 'eggs.moveParents' : 'eggs.selectOffspring')}</p>}
    {c.kind === 'egg_laying' && c.genotype_review_required && <p className="notice warning">{t('eggs.genotypeReview')}</p>}
    ...
    {c.injection && <InjectionDetails culture={c} state={state} select={select} perform={taskProps.perform}/>}
    ...
    {!hourly && !injectionChild && <section><div className="panel-title"><h3>{t('eggs.linkedContainers')}</h3>{['active','planned'].includes(c.status) && <Button variant="outline" onClick={() => dialog({kind:'egg-from', culture:c})}>{t('eggs.fromContainer')}</Button>}</div>{state.containers.filter(x => x.source_id === c.id && x.kind === 'egg_laying').map(x => <p key={x.id}><button className="text-link" onClick={() => select(x.id)}>{x.label}</button> · {t(`status.${x.status}`)}</p>)}</section>}
    ...
    {c.kind === 'egg_laying' && <section><div className="panel-title"><h3>{t('eggs.batches')}</h3><Button disabled={c.status !== 'active' || c.parents !== 'present' || c.genotype_review_required || c.setup_time_review_required} onClick={() => dialog({kind:'egg-batch',culture:c})}>{t('eggs.new')}</Button></div>{state.egg_batches.filter(b => b.source_id === c.id).map(b => <div className="egg-batch" key={b.id}><h4>{b.label} <span className="pill neutral">{t(`eggs.status.${b.status}`)}</span></h4><p>{fmtDate(b.lay_start)} · {fmtTime(b.lay_start)} → {fmtDate(b.lay_end)} · {fmtTime(b.lay_end)} · {b.temperature}°C</p><p className="subtle">{b.genotype}</p>{b.notes && <p>{b.notes}</p>}<div className="operation-grid">{b.status === 'planned' && <Button onClick={() => dialog({kind:'egg-action',batch:b,action:'collect'})}>{t('eggs.collect')}</Button>}{b.status === 'collected' && <><Button onClick={() => dialog({kind:'container',eggBatch:b})}>{t('eggs.prepare')}</Button><Button variant="outline" onClick={() => dialog({kind:'egg-action',batch:b,action:'use'})}>{t('eggs.use')}</Button><Button variant="outline" disabled={c.status !== 'active'} onClick={() => dialog({kind:'reminder',batch:b})}>{t('common.newTask')}</Button></>}{b.status === 'planned' && <Button variant="outline" onClick={() => dialog({kind:'egg-action',batch:b,action:'cancel'})}>{t('eggs.cancel')}</Button>}</div>{state.containers.filter(d => d.egg_batch_id === b.id).map(d => <p key={d.id}><button className="text-link" onClick={() => select(d.id)}>{d.label}</button> · {t(`purpose.${d.purpose}`)}</p>)}{b.uses.map(u => <p key={u.id}>{fmtDate(u.at)} · {fmtTime(u.at)} — {t(`purpose.${u.purpose}`)}{u.notes ? ` · ${u.notes}` : ''}</p>)}</div>)}</section>}
    {c.status === 'active' && !hourly && !injectionChild && <section><h3>{t('container.operations')}</h3><div className="operation-grid"><Button variant="outline" disabled={c.parents !== 'present' || c.transfer_index >= c.template.max_transfers} onClick={() => dialog({kind: 'container', source: c, mode: 'transfer'})}>{t('container.transfer')}</Button>{!c.injection && <Button variant="outline" onClick={() => dialog({kind: 'container', source: c, mode: c.purpose === 'stock' ? 'renew' : 'generation'})}>{t(c.purpose === 'stock' ? 'task.stock' : 'container.nextGeneration')}</Button>}
      {!c.injection && <Button variant="outline" onClick={() => dialog({kind: 'injection-start', culture: c})}>{t('injection.start')}</Button>}
      {['remove', 'clear', ...(collectsVirgins ? ['collect'] : !c.injection && c.purpose === 'cross' && c.workflow?.cross_goal === 'score' ? ['score'] : []), 'third_instar', 'larvae', 'pupae', 'eclosion', ...(c.kind === 'bottle' ? ['tissue'] : []), c.temperature === 18 ? 'warm' : 'cold'].map(action => <Button key={action} variant="outline" disabled={(action === 'remove' && c.parents !== 'present') || (action === 'cold' && c.temperature_policy === 'forbidden')} onClick={() => dialog({kind: 'action', culture: c, action})}>{t(`action.${action}`)}</Button>)}</div>
      {!c.injection && c.purpose !== 'stock' && <Button className="planner-button" variant="outline" onClick={() => dialog({kind: 'planner', culture: c})}><Snowflake size={16}/>{t('planner.open')}<ArrowRight size={16}/></Button>}</section>}
    {c.status === 'planned' && !injectionChild && <div className="operation-grid"><Button onClick={() => dialog({kind: 'action', culture: c, action: 'activate'})}>{t(c.kind === 'egg_laying' ? 'eggs.recordStart' : 'action.activate')}</Button>{!hourly && <Button variant="outline" onClick={() => dialog({kind: 'planner', culture: c})}>{t('planner.setupFind')}</Button>}</div>}
