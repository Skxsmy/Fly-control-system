...
type DialogState = {kind: 'delete-activity'; culture: Culture; activity: ActivityRecord} | {kind: 'container'; source?: Culture; eggBatch?: EggBatch; mode?: 'transfer' | 'generation' | 'renew' | 'edit'} | {kind: 'action'; culture: Culture; action: string; event?: Task} | {kind: 'reminder'; event?: Task; cultureId?: string; date?: string; batch?: EggBatch} | {kind: 'availability'; date: string} | {kind: 'planner'; culture: Culture} | {kind: 'egg-batch'; culture: Culture} | {kind: 'egg-from'; culture: Culture} | {kind: 'delete'; culture: Culture} | {kind: 'egg-action'; batch: EggBatch; action: 'collect' | 'use' | 'cancel'};
...
function TaskList({events, containers, perform, move, update, select, busy, empty, showDate}: TaskProps & {events: Task[]; empty?: React.ReactNode; showDate?: boolean}) {
  ...
  return <div className="task-list">{events.map(e => {const c = containers.find(c => c.id === e.container_id); const spansDays = e.due.slice(0, 10) !== e.end.slice(0, 10); return <div className={`task-row ${e.critical && e.status === 'pending' ? 'critical' : ''} ${e.status !== 'pending' ? 'resolved' : ''}`} key={e.id}>
    <div className="task-time">{(showDate || spansDays) && <span>{fmtDate(e.due)}</span>}<strong>{fmtTime(e.due)}{!spansDays && e.end !== e.due ? `–${fmtTime(e.end)}` : ''}</strong>{spansDays && <small>→ {fmtDate(e.end)} · {fmtTime(e.end)}</small>}</div>
...
function ContainerRows({culture: c, now, next, select, group}: {culture: Culture; now: string; next?: Task; select: (id: string) => void; group?: string}) {return <>{group && <tr className="group-row"><th colSpan={6}>{group}</th></tr>}<tr>
  ...
  <td className="genotype-cell"><span>{genotype(c)}</span><small>{t(`purpose.${c.purpose}`)}</small></td><td><strong className="mono">{c.kind === 'petri_dish' ? t('eggs.ageShort', {min:fmtNumber(c.egg_age_hours?.[0] || 0), max:fmtNumber(c.egg_age_hours?.[1] || 0)}) : c.kind === 'egg_laying' ? eggElapsed(c, now) : t('container.day', {day: c.calendar_day})}</strong><small>{fmtDate(c.setup_date)}{c.kind === 'egg_laying' && c.setup_time ? ` · ${c.setup_time}` : ''}</small></td>
...
function CultureDetails({culture: c, state, taskProps, dialog, select, askAssistant}: {culture: Culture; state: AppState; taskProps: TaskProps; dialog: (d: DialogState) => void; select: (id: string) => void; askAssistant: (id: string) => void}) {
  ...
  const collectsVirgins = c.purpose === 'virgin' || (c.purpose === 'cross' && (!c.workflow || c.workflow.cross_goal === 'virgins'));
  return <div className="culture-detail"><div className="detail-badges"><span className="pill green">{t(`status.${c.status}`)}</span><span className="pill neutral">{t(`purpose.${c.purpose}`)}</span><span className={`pill ${c.temperature === 18 ? 'blue' : 'neutral'}`}>{c.temperature}°C</span><Button variant="ghost" onClick={() => dialog({kind: 'container', source: c, mode: 'edit'})}><Settings2 size={16}/>{t('common.edit')}</Button></div>
    <div className="detail-clock"><div><span>{t(c.kind === 'petri_dish' ? 'eggs.ageLabel' : c.kind === 'egg_laying' ? 'eggs.elapsedLabel' : 'container.age')}</span><strong>{c.kind === 'petri_dish' ? t('eggs.ageShort', {min:fmtNumber(c.egg_age_hours?.[0] || 0), max:fmtNumber(c.egg_age_hours?.[1] || 0)}) : c.kind === 'egg_laying' ? eggElapsed(c, state.now) : t('container.day', {day: c.calendar_day})}</strong>{!hourly && <small>{t('container.effective', {age: fmtNumber(c.effective_age)})}</small>}</div>{c.kind !== 'petri_dish' && <div><span>{t('container.transfers')}</span><strong>{c.transfer_index}{!hourly && <small> / {c.template.max_transfers}</small>}</strong><small>{t(c.kind === 'egg_laying' && c.status === 'planned' ? 'eggs.adultsPending' : `parents.${c.parents}`)}</small></div>}</div>
    <dl className="detail-facts"><div><dt>{t(c.kind === 'petri_dish' ? 'eggs.dishSetup' : c.kind === 'egg_laying' ? c.status === 'planned' ? 'eggs.plannedStart' : 'eggs.actualStart' : 'container.setup')}</dt><dd>{fmtDate(c.setup_date, {year: 'numeric', month: 'short', day: 'numeric'})}{c.setup_time ? ` · ${c.setup_time}` : ''}</dd></div><div><dt>{t('container.stage')}</dt><dd>{t(`stage.${c.stage}`)}</dd></div><div><dt>{t('temperature.policy')}</dt><dd>{t(`temperature.${c.temperature_policy}`)}</dd></div>{source && <div><dt>{t('container.source')}</dt><dd><button className="text-link" onClick={() => select(source.id)}>{source.label}</button></dd></div>}</dl>
    {c.source_relation && <p className="subtle">{t('eggs.sourceRelation')}: {t(c.adult_source === 'parents' || ['egg_laying_transfer','egg_laying_parents'].includes(c.source_relation) ? 'eggs.moveParents' : 'eggs.selectOffspring')}</p>}
