...
const preparationAge = (c: Culture) => c.injection?.started_at ? t('container.day', {day: c.injection_day ?? 0}) : t('injection.awaitingCollection');
          ...
          {page === 'today' && <>
            ...
            {state.containers.length === 0 ? <section className="welcome panel"><div className="culture-symbol"><FlaskConical size={38}/></div><h2>{t('today.welcome')}</h2><Button onClick={() => setDialog({kind: 'container'})}><Plus size={18}/>{t('common.newContainer')}</Button></section> : <div className="today-layout"><div>
              ...
              <section className="panel"><div className="panel-title"><h2>{t('today.queue')}<span className="count-badge">{pending.filter(e => isTodayWork(e, state.now)).length}</span></h2><Button variant="ghost" onClick={() => setDialog({kind: 'reminder'})}><Plus size={16}/>{t('common.newTask')}</Button></div>
                <TaskList {...taskProps} events={pending.filter(e => isTodayWork(e, state.now))} empty={<Empty title={t('today.empty')}/>}/>
              </section>
...
function TaskList({events, containers, perform, move, update, select, busy, empty, showDate}: TaskProps & {events: Task[]; empty?: React.ReactNode; showDate?: boolean}) {
  ...
  return <div className="task-list">{events.map(e => {const c = containers.find(c => c.id === e.container_id); const spansDays = e.due.slice(0, 10) !== e.end.slice(0, 10); return <div className={`task-row ${e.critical && e.status === 'pending' ? 'critical' : ''} ${e.status !== 'pending' ? 'resolved' : ''}`} key={e.id}>
    <div className="task-time">{(showDate || spansDays) && <span>{fmtDate(e.due)}</span>}<strong>{e.all_day ? t('task.allDay') : <>{fmtTime(e.due)}{!spansDays && e.end !== e.due ? `–${fmtTime(e.end)}` : ''}</>}</strong>{spansDays && <small>→ {fmtDate(e.end)} · {fmtTime(e.end)}</small>}</div>
...
function ContainerRows({culture: c, now, next, select, group}: {culture: Culture; now: string; next?: Task; select: (id: string) => void; group?: string}) {return <>{group && <tr className="group-row"><th colSpan={6}>{group}</th></tr>}<tr>
  ...
  <td className="genotype-cell"><span>{genotype(c)}</span><small>{t(`purpose.${c.purpose}`)}</small></td><td><strong className="mono">{isInjectionChild(c) ? preparationAge(c) : c.kind === 'petri_dish' ? t('eggs.ageShort', {min:fmtNumber(c.egg_age_hours?.[0] || 0), max:fmtNumber(c.egg_age_hours?.[1] || 0)}) : c.kind === 'egg_laying' ? eggElapsed(c, now) : t('container.day', {day: c.calendar_day})}</strong>{(!isInjectionChild(c) || c.injection?.started_at) && <small>{fmtDate(c.setup_date)}{(c.kind === 'egg_laying' || isInjectionChild(c)) && c.setup_time ? ` · ${c.setup_time}` : ''}</small>}</td>
...
function CultureDetails({culture: c, state, taskProps, dialog, select, askAssistant}: {culture: Culture; state: AppState; taskProps: TaskProps; dialog: (d: DialogState) => void; select: (id: string) => void; askAssistant: (id: string) => void}) {
  ...
  return <div className="culture-detail"><div className="detail-badges"><span className="pill green">{t(`status.${c.status}`)}</span><span className="pill neutral">{t(`purpose.${c.purpose}`)}</span><span className={`pill ${c.temperature === 18 ? 'blue' : 'neutral'}`}>{c.temperature}°C</span><Button variant="ghost" onClick={() => dialog({kind: 'container', source: c, mode: 'edit'})}><Settings2 size={16}/>{t('common.edit')}</Button></div>
    <div className="detail-clock"><div><span>{t(injectionChild ? 'injection.elapsed' : c.kind === 'petri_dish' ? 'eggs.ageLabel' : c.kind === 'egg_laying' ? 'eggs.elapsedLabel' : 'container.age')}</span><strong>{isInjectionChild(c) ? preparationAge(c) : c.kind === 'petri_dish' ? t('eggs.ageShort', {min:fmtNumber(c.egg_age_hours?.[0] || 0), max:fmtNumber(c.egg_age_hours?.[1] || 0)}) : c.kind === 'egg_laying' ? eggElapsed(c, state.now) : t('container.day', {day: c.calendar_day})}</strong>{!hourly && !injectionChild && <small>{t('container.effective', {age: fmtNumber(c.effective_age)})}</small>}</div>{c.kind !== 'petri_dish' && !injectionChild && <div><span>{t('container.transfers')}</span><strong>{c.transfer_index}{!hourly && <small> / {c.template.max_transfers}</small>}</strong><small>{t(c.kind === 'egg_laying' && c.status === 'planned' ? 'eggs.adultsPending' : `parents.${c.parents}`)}</small></div>}</div>
