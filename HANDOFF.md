...
- Define each new rule's **anchor, clock basis, target and units, collection window, and manual-override policy** explicitly. Fixed calendar/elapsed-hour rules use `elapsed_target` (unit-rate timeline), not the culture temperature multiplier. Existing parent transfer/removal and stock renewal remain calendar-based. Egg-laying/Petri-dish timing remains anchored to actual/planned laying intervals without automatic 18°C conversion. A new protocol must be defined by the researcher before choosing a new rate or timing anchor.
